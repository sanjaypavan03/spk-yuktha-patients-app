...
export async function POST(request: NextRequest) {
  try {
    ...
    // Robust local midnight calculation (IST default)
    const tzOffset = 330; // IST
    const offsetMs = tzOffset * 60 * 1000;
    const nowUtc = Date.now();
    const localMidnight = new Date(Math.floor((nowUtc + offsetMs) / 86400000) * 86400000 - offsetMs);

    const start = startDate ? new Date(startDate) : localMidnight;
    if (startDate) {
        // If user provided a date, normalize it to the same local midnight logic
        start.setHours(0, 0, 0, 0); 
        // Note: For true precision we'd use the offset here too, but this is usually close enough for start dates
    }
