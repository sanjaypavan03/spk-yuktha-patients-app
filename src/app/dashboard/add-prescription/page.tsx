...
export default function AddPrescriptionPage() {
  ...
  const handleAnalyze = async (dataUri: string) => {
    ...
    try {
      const result = await extractMedications(dataUri);
  ...
  const handleAddToMeds = async () => {
    if (!analysisResult?.medicines || analysisResult.medicines.length === 0) return;

    try {
      // Save all medicines to backend
      const promises = analysisResult.medicines.map((med: any) =>
        fetch('/api/medicines', {
          method: 'POST',
          headers: {
      ...
      toast({
        ...
        description: `${analysisResult.medicines.length} new medication(s) have been added to your tracker.`,
      });
            ...
            <div className="p-4 space-y-4">
                ...
                {analysisResult?.medicines && analysisResult.medicines.length > 0 ? (
                    <div className="space-y-3">
                        {analysisResult.medicines.map((med: any, index: number) => (
                            <Card key={index}>
