...
export default function EmergencyHubScreen() {
    ...
    const [yukthaId, setYukthaId] = useState('Loading...');
    ...
    const fetchMedicalInfo = async () => {
        try {
            let id = user?.yukthaId || user?.qrCode || '';
            
            // BUG 3 Fix: Handle old accounts with missing yukthaId
            if (!id || id === 'NOT-ASSIGNED') {
                ...
                if (typeof updateUser === 'function') {
                    await updateUser({ yukthaId: id, qrCode: id });
                }
            }
            setYukthaId(id);
            ...
            if (id) {
                ...
                if (res.ok) {
                    ...
                    // No profile yet, but we still want to show the empty state
                    setMedicalInfo({});
                }
            }
        } catch (error) {
            console.error("Failed to fetch medical info:", error);
            // Even on error, ensure we have an empty object for rendering
            if (!medicalInfo) setMedicalInfo({});
        } finally {
            setLoading(false);
        }
    };
    ...
    // Use state-based yukthaId instead of local const redeclaration

    const qrPayload = JSON.stringify({
        platform: 'yuktha.health',
        patientId: yukthaId,
        name: user?.name || '',
        tier1: {
            blood:      medicalInfo?.bloodGroup || 'Unknown',
            allergy:    medicalInfo?.knownAllergies ? (medicalInfo?.allergiesDetails || 'Yes') : 'None',
            conditions: medicalInfo?.chronicConditions || 'None',
            meds:       medicines.length > 0
                        ? medicines.map((m: any) => `${m.name} ${m.dosage}`).join(', ')
                        : (medicalInfo?.currentMedications || 'None'),
            contact:    medicalInfo?.emergencyContact1Name
                        ? `${medicalInfo.emergencyContact1Name} · ${medicalInfo.emergencyContact1Phone}`
                        : 'Not set',
            pacemaker:  medicalInfo?.hasPacemakerOrImplant ? 'YES' : 'NO',
            pregnant:   medicalInfo?.isPregnant ? 'YES' : 'NO',
        },
        tier2: 'Login to Yuktha portal → search by Patient ID',
    });
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} bounces={true}>
                ...
                <View style={styles.main}>
                    <View style={styles.idCardContainer}>
                        <BlurView intensity={Platform.OS === 'ios' ? 80 : 100} tint="dark" style={styles.idBlur}>
                            <Text style={styles.idLabel}>YUKTHA PATIENT ID</Text>
                            <View style={styles.idValueBox}>
                                <Text style={styles.idValue}>{yukthaId}</Text>
                            </View>
                            <Text style={styles.idSubtext}>
                                This is your unique, permanent identity within the Yuktha Health network.
                            </Text>
                            <View style={styles.idActionRow}>
                                <TouchableOpacity 
                                    style={styles.idActionBtn}
                                    onPress={() => {
                                        Clipboard.setStringAsync(yukthaId);
                                        Alert.alert('Copied', 'Patient ID copied to clipboard');
                                    }}
                                >
                                    <ShieldCheck size={18} color="#10B981" />
                                    <Text style={styles.idActionText}>Copy</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                    style={styles.idActionBtn}
                                    onPress={() => Share.share({
                                        message: `My Yuktha Patient ID: ${yukthaId}. Use this in the Yuktha Health portal.`
                                    })}
                                >
                        ...
                        {loading ? (
                            ...
                            <View>
                                <View style={styles.infoList}>
                                    ...
                                    <View style={styles.infoCard}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={[
                                                styles.infoValue, 
                                                !medicalInfo?.bloodGroup && { color: '#F43F5E', fontStyle: 'italic' },
                                                medicalInfo?.bloodGroup && styles.infoValueLarge
                                            ]}>
                                                {medicalInfo?.bloodGroup || 'Not set — tap Edit to add'}
                                            </Text>
                                        </View>
                                    </View>
                                    ...
                                    <View style={styles.infoCard}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={styles.infoValue}>
                                                {medicalInfo?.knownAllergies ? (medicalInfo?.allergiesDetails || 'Yes') : 'None reported'}
                                            </Text>
                                        </View>
                                    </View>
                                    ...
                                    <View style={styles.infoCard}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={styles.infoValue}>
                                                {medicalInfo?.chronicConditions || 'None reported'}
                                            </Text>
                                        </View>
                                    </View>
                                ...
                                {medicalInfo?.emergencyContact1Name ? (
                                    ...
                                    <View style={[styles.contactCard, { borderStyle: 'dashed', borderColor: '#F43F5E', backgroundColor: 'transparent' }]}>
                                        <View style={{ flex: 1 }}>
                                            <Text style={styles.contactLabelSmall}>Primary Emergency Contact</Text>
                                            <Text style={{ color: '#F43F5E', fontStyle: 'italic', fontWeight: 'bold' }}>Not set — tap Edit to add</Text>
                                        </View>
                                    </View>
                                )}
                            </View>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <View style={{ paddingHorizontal: 16, paddingVertical: 10, 
                            ...
                            <Text style={{ fontSize: 9, fontWeight: '900', 
                                        letterSpacing: 2, color: '#94A3B8' }}>PATIENT ID</Text>
                            <Text style={{ fontSize: 13, fontWeight: '900', 
                                        color: '#10B981', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace' }}>
                                {yukthaId}
                            </Text>
                        </View>
